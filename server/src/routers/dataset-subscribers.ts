...
import type { DatasetSubscribersStore, DatasetSubscriberType } from "../stores/dataset-subscribers-store/types.js";
...
function asSubscriberType(value: unknown): DatasetSubscriberType | undefined {
...
export function createDatasetSubscribersRouter(store: DatasetSubscribersStore): Router {
  const router = Router();
  ...
  router.patch("/:type", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      const updated = await store.read(uid, type);
      res.json(updated);
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : "Unauthorized";
      if (isServerMisconfiguredAuthError(message)) {
